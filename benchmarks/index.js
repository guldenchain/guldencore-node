...
console.log('Gulden Service native interface vs. Gulden JSON RPC interface');
console.log('----------------------------------------------------------------------');

// To run the benchmarks a fully synced Bitcore Core directory is needed. The RPC comands
// can be modified to match the settings in Gulden.conf.
...
var bitcoind = require('../').services.Bitcoin({
  node: {
    datadir: process.env.HOME + '/.gulden',
    network: {
      name: 'testnet'
    }
  }
...
bitcoind.start(function(err) {
  ...
  console.log('Gulden Core started');
});

bitcoind.on('ready', function() {

  console.log('Gulden Core ready');
