...
/**
 * Provides a friendly event driven API to GuldenD in Node.js. Manages starting and
 * stopping GuldenD as a child process for application support, as well as connecting
 * to multiple GuldenD processes for server infrastructure. Results are cached in an
...
Bitcoin.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'GuldenD/' + name, 'total:', this.subscriptions[name].length);
};
...
Bitcoin.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'GuldenD/' + name, 'total:', this.subscriptions[name].length);
};
...
Bitcoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'GuldenD/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Bitcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'GuldenD/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in GuldenD config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in GuldenD config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in GuldenD config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './Gulden.conf');

  log.info('Using Gulden config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of guldencore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of guldencore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of guldencore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to Guldend from guldencore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from GuldenD. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from GuldenD. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in Gulden.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that GuldenD is undergoing a reindex. ' +
             ...
             'of guldencore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/Gulden.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/Gulden.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('GuldenD/hashblock', message.toString('hex'));
    }
  }
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Gulden Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('GuldenD/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('GuldenD/rawtransaction', message.toString('hex'));
    }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Gulden Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/GuldenD.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned Gulden process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean Gulden process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting Gulden process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Gulden process unexpectedly exited with code:', code);
        log.warn('Restarting Gulden child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Gulden process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new BitcoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 9232,
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn GuldenD.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to GuldenD.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Gulden configuration options "spawn" or "connect" are expected'));
    }
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('GuldenD spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('GuldenD process did not exit'));
      }
