...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'guldencore-lib': '^' + bitcore.version,
    'guldencore-node': version
  }
};

/**
 * Will create a directory and Gulden.conf file for Bitcoin.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/guldencore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the guldencore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the guldencore-node directory and configuration
