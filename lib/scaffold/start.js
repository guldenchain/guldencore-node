...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for GuldenD is necessary for this upgrade with the "reindex=1" Gulden.conf option. \n' +
                 'There are changes necessary in both Gulden.conf and guldencore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed guldencore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './guldencore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in guldencore-node services directory
